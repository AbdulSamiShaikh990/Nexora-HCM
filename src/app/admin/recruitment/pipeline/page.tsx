...
interface Application { id: string; jobId: string; candidateId: string; stage: Stage; scorePercent?: number; passed?: boolean; }
...
export default function PipelinePage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!cr.ok) throw new Error(c?.error || "Failed candidates");
      if (!ar.ok) throw new Error(a?.error || "Failed applications");
      setJobs(j); setCands(c); setApps(a);
    } catch (e: any) {
